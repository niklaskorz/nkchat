 ...
 * Whether or not the PubSubAsynIterator is in listening mode (responding to incoming
 * PubSubEngine events and next() calls).
...
export default class PubSubAsyncIterator<T> implements AsyncIterator<T> {
  ...
  private async pushValue(event: T) {
    await this.allSubscribed;
    const head = this.pullQueue.shift();
    if (head) {
      head({ value: event, done: false });
    } else {
